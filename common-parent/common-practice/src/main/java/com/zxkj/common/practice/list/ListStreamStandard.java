...
public class ListStreamStandard {
...
//        random3.ints().limit(10).sorted().forEach(System.out::println);
        ...
        // parallel(parallelStream 是流并行处理程序的代替方法。以下实例我们使用 parallelStream 来输出空字符串的数量)
        List<String> strings3 = Arrays.asList("abc", "", "bc", "efg", "abcd", "", "jkl");
        // 获取空字符串的数量
        strings3.parallelStream().filter(string -> string.isEmpty()).count();
