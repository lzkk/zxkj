...
public abstract class IDatabaseQuery {
    ...
    public static class DefaultDatabaseQuery extends IDatabaseQuery {
        ...
        {
            jdbcTypeMap.put("BIT","TINYINT");
            jdbcTypeMap.put("LONGVARCHAR","VARCHAR");
        }
        ...
        private void convertTableFields(TableInfo tableInfo) {
            ...
            try {
                ...
                dbQuery.query(tableFieldsSql, result -> {
                    ...
                    TableField.MetaInfo metaInfo = new TableField.MetaInfo(columnsMetaInfoMap.get(columnName.toLowerCase()));
                    field.setMetaInfo(metaInfo);
                    if(dbType.getDb().equals("mysql")){
                        String jdbcTypeStr = metaInfo.getJdbcType().toString();
                        if(jdbcTypeMap.containsKey(jdbcTypeStr)){
                            jdbcTypeStr = jdbcTypeMap.get(jdbcTypeStr);
                        }
                        field.setType(jdbcTypeStr);
